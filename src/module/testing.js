...
const getLinksValidated = (markdown) => {
    ...
    for (let i = 0; i < markdown.length; i++) {
        if (match !== null) {
            const status = fetch(match[4]).then((response) => {
...
//         console.error(error + 'Error: file not found');
//     });
const readFileValidated = (filePath) => fs.readFile(filePath, 'utf8', (error, data) => {
    if (error) {
        console.error(error + 'Error: file not found');
    } else {
        console.log(getLinksValidated(data));
...
readFileValidated('sample_folder/sample_two.md');
